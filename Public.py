#!/usr/bin/env python
# -*- coding: utf-8 -*-

import requests
import json

try:
    from urlparse import urljoin
except ImportError:
    from urllib.parse import urljoin
    
class UserApi(object):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        self.cookie=""
        self.User_login("admin", "123456")
    def User_login(self, username, password):
        url = urljoin(self.base_url, '/api/users/login')
        data = {
            'username': username,
            'password': password
        }
        response = requests.post(url, json.dumps(data),headers=self.headers)
        self.cookie=response.cookies
        return response
 
    def User_list(self):
        url = urljoin(self.base_url, '/api/users')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def User_get(self):
        url = urljoin(self.base_url, '/api/users/admin')
        response = requests.get(url,cookies=self.cookie, headers=self.headers)
        return response
    
    def User_any(self):
        url = urljoin(self.base_url, '/api/users/any')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
  
    def User_add(self):
        url = urljoin(self.base_url, '/api/users')
        data = {
            "name": "admin",
            "password": "123456",
            "desc": "the only privileged user",
         }
        response = requests.post(url, json.dumps(data),headers=self.headers)
        return response
  
    def User_update(self):
        url = urljoin(self.base_url, '/api/users/admin')
        data = {"desc":"new1  new2 new3 description text"}
        response = requests.patch(url,json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
  
    def User_password(self):
        url = urljoin(self.base_url, '/api/users/admin/change_password')
        data = {"old": "123456","new": "123456"}
        response = requests.patch(url,json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def User_out(self):
        url = urljoin(self.base_url, '/api/users/logout')
        response = requests.delete(url,cookies=self.cookie,headers=self.headers)
        return response
  
class NodeApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
        
    
    def Node_list(self):
        url = urljoin(self.base_url, '/api/nodes')
        response = requests.get(url,cookies=self.cookie, headers=self.headers)
        return response
    
    def Node_get(self):
        url = urljoin(self.base_url, '/api/nodes/6e28d82ca8f14a1e')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_add(self):
        url = urljoin(self.base_url, '/api/nodes?exec_id=1111122222')
        data = {"addr": "192.168.1.244:22","user": "root","password": "123","privkey": "","timeout": 10}
        response = requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_set(self):
        url = urljoin(self.base_url, '/api/nodes/10badfbffc60bd5e/attrs')
        data= {
              "manage_ip": "192.168.1.244",
              "internal_ip": "10.0.0.5",
              "env": {
                "os": "centos7",
                "zone": "bj"
              },
              "custom": {
                "admin": "xbwu@dataman-inc.com",
                "ssd": "true"
              },
              "host_name": "wxbss244"
            }
        response = requests.put(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_batchSet(self):
        url = urljoin(self.base_url, '/api/nodes/attrs')
        csvfile ='D:\\Project\\456.csv'
        with open(csvfile,'r') as f:
            data = f.read()
        response = requests.put(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_watch(self):
        url = urljoin(self.base_url, 'api/nodes/95955be4b4a57335/events')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_exec(self):
        url = urljoin(self.base_url, '/api/nodes/95955be4b4a57335/exec')
        data = {"command": "for((i=1;i<=1;i++));do sleep 1;echo $i;done"}
        response = requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_delete(self):
        url = urljoin(self.base_url, '/api/nodes/95955be4b4a57335')
        response = requests.delete(url,cookies=self.cookie,headers=self.headers)
        return response
    
    
    def Node_upgrade(self):
        url = urljoin(self.base_url, '/api/nodes/95955be4b4a57335/upgrade')
        response = requests.put(url,cookies=self.cookie,headers=self.headers)
        return response
    
    
    def Node_alabel(self):
        url = urljoin(self.base_url, '/api/nodes/95955be4b4a57335/labels')
        data ={"name": "wxb123", "sex": "Man22123"}
        response = requests.put(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response

    def Node_ulabel(self):
        url = urljoin(self.base_url, '/api/nodes/0fc53038866156f3/labels')
        data ={"name": "wxb123", "sex": "Man22123"}
        response = requests.delete(url, data=json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_lyum(self):
        url = urljoin(self.base_url, '/api/nodes/6e28d82ca8f14a1e/repos')
        response = requests.get(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_ayum(self):
        url = urljoin(self.base_url, '/api/nodes/6e28d82ca8f14a1e/repos?name=demorepo')
        data = [
            {
                    "title": "docker",
                    "name": "docker stable repo",
                    "baseurl": "https://download.docker.com/centos/7/stable",
                    "gpgcheck":False,
            },
                {
                    "title": "update",
                    "name": "docker update repo",
                    "baseurl": "https://download.docker.com/centos/7/update",
                    "gpgcheck":True,
                    "gpgkey": "http://download.docker.com/centos/7/update/RPM-GPG-KEY-7-DOCKER"
                }
             ]
        response = requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Node_dlabel(self):
        url = urljoin(self.base_url, '/api/nodes/6e28d82ca8f14a1e/repos?name=demorepo')
        response = requests.delete(url,cookies=self.cookie,headers=self.headers)
        return response
    
class ClusterApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
    
    def Cluster_list(self):
        url = urljoin(self.base_url, '/api/clusters')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Cluster_get(self):
        url = urljoin(self.base_url, '/api/clusters/demo123')
        response = requests.get(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Cluster_add(self): 
        url = urljoin(self.base_url, '/api/clusters')
        data = {
            "name": "demo123",
            "desc": "demo123 cluster",
            "attrs": {
                "admin": "admin@demo.net",
                "zone": "bj"
                    }
                } 
        response = requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
        
    
    def Cluster_update(self): 
        url = urljoin(self.base_url, '/api/clusters/demo123')
        data = {
            "desc": "demo2 cluster",
            "attrs": {
                "admin": "admin2@demo.net",
                "zone": "bj111111111111"
                    }
            }
        response = requests.patch(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Cluster_delete(self): 
        url = urljoin(self.base_url, '/api/clusters/demo123')
        response = requests.delete(url,cookies=self.cookie,headers=self.headers)
        return response
          
    
    def Cluster_assign(self): 
        url = urljoin(self.base_url, '/api/clusters/demo123/0fc53038866156f3')
        response = requests.put(url,cookies=self.cookie,headers=self.headers)
        return response
    
    
    def Cluster_uassign(self): 
        url = urljoin(self.base_url, '/api/clusters/demo123/0fc53038866156f3')
        response = requests.delete(url,cookies=self.cookie,headers=self.headers)
        return response
    
class JobApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
    
    def Job_list(self):
        url = urljoin(self.base_url, '/api/jobs')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Job_get(self):
        url = urljoin(self.base_url, '/api/jobs/8eb3e80013e118a6')
        response = requests.get(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Job_stop(self):
        url = urljoin(self.base_url, '/api/jobs/8eb3e80013e118a6/stop')
        response = requests.patch(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Job_remove(self):
        url = urljoin(self.base_url, '/api/jobs/a312416aaab22df8')
        response = requests.delete(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Job_follow(self):
        url = urljoin(self.base_url, '/api/jobs/8eb3e80013e118a6/follow')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
     
    def Job_progress(self):
        url = urljoin(self.base_url, '/api/jobs/8eb3e80013e118a6/progress')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response

class APPApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
    
    def APP_list(self):
        url = urljoin(self.base_url, '/api/apps')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_get(self):
        url = urljoin(self.base_url, '/api/apps/nginx')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_add(self):
        url = urljoin(self.base_url, '/api/apps')
        data = {"name": "nginx",
                "desc": "nginx app",
                "version":"1.13.7",
                "release":"nginx"}
        response=requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_update(self):
        url = urljoin(self.base_url, '/api/apps/nginx')
        data = {"desc": "nginx update app"}
        response=requests.patch(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_delete(self):
        url = urljoin(self.base_url, '/api/apps/nginx')
        response = requests.delete(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_importrevisions(self):
        url = urljoin(self.base_url, '/api/apps/nginx/revisions')
        tar="D:\\Project\\app\\nginx.tar"
        with open(tar,"r") as f:
            data=f.read()
        response = requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_revisionslist(self):
        url = urljoin(self.base_url, '/api/apps/nginx/revisions')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_getrevisions(self):
        url = urljoin(self.base_url, '/api/apps/nginx/revisions/051b8bb7f1838112')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_exportrevisions(self):
        url = urljoin(self.base_url, '/api/apps/nginx/revisions/051b8bb7f1838112/export')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def APP_deploy(self):
        url = urljoin(self.base_url, '/api/apps/nginx/revisions/17e4c933dc36dddc/deploy')
        data ={
            "name": "DataMan4",
            "desc": "nginx20171225  nginx app instance",
            "binded_datas": {
                "PORT": "81"
                },
            "binded_labels": {
                "ssd": "True"
                },
            "binded_cluster": "wxb",
            "replicas": 1
            }
        response = requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    

class InstanceApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
        
        
    def Instance_list(self):
        url = urljoin(self.base_url, '/api/instances')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_get(self):
        url = urljoin(self.base_url, '/api/instances/06bf8848eb98a8af')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_update(self):
        url = urljoin(self.base_url, '/api/instances/06bf8848eb98a8af')
        data ={"desc": "修改实例节点123"}
        response = requests.patch(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_start(self):
        url = urljoin(self.base_url, '/api/instances/06bf8848eb98a8af/start')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response
     
    def Instance_reload(self):
        url = urljoin(self.base_url, '/api/instances/06bf8848eb98a8af/reload')
        data ={"PORT": "88"}
        response = requests.patch(url,json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response

    def Instance_log(self):
        url = urljoin(self.base_url, '/api/instances/DataMan3?script=install') 
        # detect|install|uninstall|reinstall|start|stop|reload|upgrade
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_stop(self):
        url = urljoin(self.base_url, '/api/instances/06bf8848eb98a8af/stop')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_restart(self):
        url = urljoin(self.base_url, '/api/instances/06bf8848eb98a8af/restart')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_apply(self):
        url = urljoin(self.base_url, '/api/instances/06bf8848eb98a8af/apply?revision_id=06bf8848eb98a8af')
        data ={"PORT": "88","USER": "admin"}
        response = requests.patch(url,json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_events(self):
        url = urljoin(self.base_url, '/api/instances/events?instance_id=DataMan3') 
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_remove(self):
        url = urljoin(self.base_url, '/api/instances/DataMan2?force=true')
        response = requests.delete(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Instance_scalelogs(self):
        url = urljoin(self.base_url, '/api/instances/DataMan2/scalelogs') 
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response

class TaskApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
        
    def Task_list(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Task_get(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks/e6647542687efc90')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Task_start(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks/e6647542687efc90/start')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Task_stop(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks/e6647542687efc90/stop')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Task_restart(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks/e6647542687efc90/restart')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response

    def Task_log(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks/e6647542687efc90/log?script=install') 
        # detect|install|uninstall|reinstall|start|stop|restart|reload|upgrade|statecheck|enable|disable
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Task_enable(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks/e6647542687efc90/enable')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response
    
    def Task_disable(self):
        url = urljoin(self.base_url, '/api/instances/nginx/tasks/e6647542687efc90/disable')
        response = requests.patch(url,cookies=self.cookie,headers=self.headers)
        return response
    
      
class SshkeyApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
    
    def Sshkey_list(self):
        url = urljoin(self.base_url, '/api/sshkeys')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Sshkey_get(self):
        url = urljoin(self.base_url, '/api/sshkeys/demo1')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
     
    def Sshkey_add(self):
        url = urljoin(self.base_url, '/api/sshkeys')
        data={
            "name": "DataMan3",
            "desc": "DataMan3 ssh key",
            "privkey": '''
        -----BEGIN RSA PRIVATE KEY-----
        
        -----END RSA PRIVATE KEY-----
        ''',
            }
        response = requests.post(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Sshkey_update(self):
        url = urljoin(self.base_url, '/api/sshkeys/DataMan3')
        data = { "desc": "new new new DataMan3 ssh key" }
        response=requests.patch(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Sshkey_remove(self):
        url = urljoin(self.base_url, '/api/sshkeys/DataMan3')
        response = requests.delete(url, cookies=self.cookie,headers=self.headers)
        return response
    
class SettingApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
    
    def Setting_get(self):
        url = urljoin(self.base_url, '/api/settings')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Setting_update(self):
        url = urljoin(self.base_url, '/api/settings')
        data ={
          "advertise_addr": "192.168.1.241:80",
          "log_level": "debug",
          "enable_auth": True,
        }
        response=requests.patch(url, json.dumps(data),cookies=self.cookie,headers=self.headers)
        return response
    
    def Setting_reset(self):
        url = urljoin(self.base_url, '/api/settings/reset')
        response=requests.put(url, cookies=self.cookie,headers=self.headers)
        return response
    
class OtherApi(UserApi):
    def __init__(self, base_url):
        self.base_url = base_url
        self.headers={'Content-Type':'application/json'}
        super().__init__(base_url)
    
    def Other_version(self):
        url = urljoin(self.base_url, '/api/version')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Other_info(self):
        url = urljoin(self.base_url, '/api/info')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Other_dump(self):
        url = urljoin(self.base_url, '/api/debug/dump?name=general')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    def Other_ping(self):
        url = urljoin(self.base_url, '/api/ping')
        response = requests.get(url, cookies=self.cookie,headers=self.headers)
        return response
    
    
       
# if __name__ == '__main__':
#     base_url = "http://192.168.1.241"
#     obj = NodeApi(base_url)
#     print(obj.Node_list())
#     
#     print(obj.User_login('admin', '123456'))